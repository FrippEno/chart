...
export async function createChart(
  ...
  return safeQuery(async () => {
    ...
    await db
      .prepare(`
        INSERT INTO charts (title, x_axis_label, y_axis_label, y_min, y_max, date_start, date_end, created_at, updated_at)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
      `)
      .bind(data.title, data.x_axis_label, data.y_axis_label, data.y_min, data.y_max, data.date_start, data.date_end, now, now)
      .run();
...
export async function updateChart(
  ...
  return safeQuery(async () => {
    ...
    for (const key of ['title', 'x_axis_label', 'y_axis_label', 'y_min', 'y_max', 'date_start', 'date_end'] as const) {
